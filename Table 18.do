...
import delimited "/Users/benediktwick/Documents/Research/Overconfidence/Confidential data set/2021 State Full Data 220629/2021 State Full Data 220629.csv"
...
*This sample is constructed by dropping self-perception>7 (variable m4>7) from NFCS. It is used for all tables that do use sp or overconfidence variables, except table SELF-ASSESSMENT AND TRUE KNOWLEDGE where we directly compare the same individuals
	...
	****Construction Credit Card Index
		...
		* Destring variables if needed (assuming these are already numeric)
	...
	****Construction Savings Index
		...
		* Ensure variables are numeric where necessary
		destring j8, replace force

		* Create new variables based on conditions without dropping observations
		generate j5_new = .
	...
	hotelling oc_1 risk credit savings budgeting, by(other)
	hotelling oc_2 risk credit savings budgeting, by(other)
	hotelling oc_3 risk credit savings budgeting, by(other)
